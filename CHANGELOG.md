...
- Flowchart AST node IDs from the Node parser bridge now preserve Mermaid case-sensitive identity (for example, `A` and `a` remain distinct IDs) while trimming incidental whitespace.
...
- Syntax-error remediation hint targeting now prefers detector-derived source locations (line/column) for tabs, smart punctuation, unterminated edge labels, and unterminated quoted labels, falling back to parser positions when detector locations are unavailable.
...
- `config.allow-unknown-rules` is now treated as a control key (not a rule ID) across versioned and legacy/flat config payloads, so it no longer triggers `unknown_option`/shape errors and still controls unknown-rule leniency (`true`) vs strict rejection (`false`).
...
- Analyze responses now include structured non-syntax `hints` for parsed-but-lint-unsupported diagram types and config-validation failures, with stable hint codes and remediation snippets while preserving existing `error` and `issues` fields.
