...
**Default behavior (lenient):** Unknown or cross-diagram-type rules are skipped and a deprecation warning is returned (HTTP 200). See [Graceful Rule Handling](#graceful-rule-handling) below.

**Strict mode:** Set `STRICT_CONFIG_SCHEMA=true` (or `1`) to opt-in to strict validation. Under strict mode, unknown rule IDs produce HTTP 400 with `error.code = unknown_rule` and `hints[]` containing suggested corrections.
...
All `/v1/*` endpoints are documented in detail in [README.md — API](README.md#api). Key grouping:

| Group | Endpoints | See Also |
|-------|-----------|----------|
| Health and status | `/healthz`, `/ready`, `/version`, `/info`, `/health/metrics`, `/diagram-types` | [README](README.md#get-v1healthz-canonical) |
| Discovery | `/rules`, `/rules/schema`, `/spec`, `/docs` | [README](README.md#get-v1rules-canonical) |
| Analysis | `/analyze`, `/analyze/raw`, `/analyze/sarif`, `/analyze/help` | [README](README.md#post-v1analyze-canonical-with-british-english-alias-post-v1analyse) |
| Operations | `/config-versions`, `/benchmark.html`, `/metrics`, `/internal/metrics` | [README](README.md#get-v1metrics) |

See the detailed endpoint reference and full examples in [README.md](README.md).
