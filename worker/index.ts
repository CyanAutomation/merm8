...
export interface Env { API_KEY?: string; MCP_ALLOWED_HOSTNAMES?: string; REST_ALLOWED_ORIGINS?: string; BUILD_VERSION?: string; BUILD_SHA?: string }
...
const hostAllowed = (request: Request, env: Env) => (env.MCP_ALLOWED_HOSTNAMES ?? "").split(",").map(x => x.trim()).includes(new URL(request.url).hostname);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === "/mcp") { if (!allowed(request, env.API_KEY) || !hostAllowed(request, env)) return json({ error: { code: "unauthorized", message: "A valid API key and allowed hostname are required" } }, 401, { "www-authenticate": "Bearer" }); return mcpHandler.fetch(request); }
    if ((path === "/v1/analyze" || path === "/v1/analyse") && request.method === "POST") {
      let body: { code?: unknown; config?: unknown }; try { body = await request.json(); } catch { return json({ error: { code: "invalid_json", message: "invalid JSON body" } }, 400); }
