...
func TestServeSpec_InfoResponseUsesOnlyKebabCase(t *testing.T) {
	...
	required := infoSchema["required"].([]interface{})
	if !slices.Contains(required, interface{}("parser-recognized")) {
		t.Fatalf("expected InfoResponse.required to include parser-recognized, got %#v", required)
	}
...
func TestOpenAPIDrift_SelectedFieldsStayInSync(t *testing.T) {
	...
	if got := lookup(t, spec, "paths", "/v1/spec", "get", "operationId"); got != "getSpec" {
		t.Fatalf("expected /v1/spec operationId getSpec, got %#v", got)
	}

	if got := lookup(t, spec, "paths", "/v1/spec", "get", "responses", "200", "content", "application/json", "schema", "type"); got != "object" {
		t.Fatalf("expected /v1/spec 200 schema type object, got %#v", got)
	}
}
