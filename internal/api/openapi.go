	...
	"paths": map[string]interface{}{
		...
		"/v1/analyze": map[string]interface{}{
			"post": map[string]interface{}{
				...
				"x-operational-env": map[string]interface{}{
					"required": []interface{}{"PARSER_CONCURRENCY_LIMIT", "PARSER_MAX_OLD_SPACE_MB"},
				},
		...
		"/v1/analyze/raw": map[string]interface{}{
			"post": map[string]interface{}{
				...
				"description": "Accepts raw mermaid code (plain text) directly in the request body. Auto-detects format: attempts to parse as JSON first (supports {\"code\": \"...\"} and optional config/parser fields), then falls back to treating the entire body as raw mermaid syntax.\n\nIf Content-Type is JSON and the payload appears JSON-like (for example starts with `{`/`[`), but decoding fails, the request is still analyzed as raw text and a request-level hint code `raw_json_decode_failed_fallback_to_text` explains that decode fallback. Non-JSON raw text sent with JSON content type still falls back successfully without that decode-fallback hint. Maximum request body size is 1 MiB.\n\nReturns the same structured analysis as /v1/analyze, including syntax errors, structured hints for fixing errors, and lint results. When syntax errors occur, the 'hints' field contains machine-readable entries for common mistakes (e.g., Graphviz syntax, tab indentation, arrow operators). The legacy 'suggestions' field remains for backward compatibility and is deprecated.",
